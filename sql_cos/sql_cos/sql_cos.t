...
结束
功能 sjcl(uxtime)
    返回 指定时间("s", uxtime, "1970-01-01 08:00:00")
结束
功能 数据_转移()
    变量 user_arr, config_arr, log_arr, cami_arr
    变量 ret = sqlitesqlarray("c:\\yxdb.db", "select * from kam_user", user_arr)
    如果(!ret)
        消息框("查询数据:" & 获取错误信息(1))
        返回 ""
    结束
    ret = sqlitesqlarray("c:\\yxdb.db", "select * from kam_config", config_arr)
    如果(!ret)
        消息框("查询数据:" & 获取错误信息(1))
        返回 ""
    结束
    ret = sqlitesqlarray("c:\\yxdb.db", "select * from kam_jilu", log_arr)
    如果(!ret)
        消息框("查询数据:" & 获取错误信息(1))
        返回 ""
    结束
    ret = sqlitesqlarray("c:\\yxdb.db", "select * from kam_kami", cami_arr)
    如果(!ret)
        消息框("查询数据:" & 获取错误信息(1))
        返回 ""
    结束
    ...
    遍历(变量 i = 0; i < 数组大小(user_arr); i++)
        ...
        变量 userinfo = 数组(value["uid"], value["pwd"], value["regtime"], value["endtime"], value["utime"], value["sunum"], value["unum"], value["mcode"])
    ...
    遍历(变量 i = 0; i < 数组大小(log_arr); i++)
        ...
        变量 userinfo = 数组(value["jtype"], value["uid"], value["jtime"], value["num"], value["msg"])
        如果(set5Log(userinfo))
            cg = cg + 1
        否则
            sb = sb + 1
