...
#pragma warning restore 618
        ...
        /// 分页(任意entity，尽量少的字段)
        /// </summary>
        public List<T> FindPageBySql<T>(string sql, string orderby, int pageSize, int currentPage, out int rows) where T : new()
        {
            using (var connection = GetConnection())
            {
                ...
                return FindListBySql<T>(GetPageSql(sql, orderby, pageSize, currentPage));
            }
        }
        #endregion
        ...
        public List<T> FindPageBySql<T>(string sql, string orderby, int pageSize, int currentPage, out int rows, params DbParameter[] cmdParms) where T : new()
        {
            using (var connection = GetConnection())
            {
                ...
                IDbCommand cmd = GetCommand(commandText, connection);
                rows = int.Parse(cmd.ExecuteScalar().ToString());
                return FindListBySql<T>(GetPageSql(sql, orderby, pageSize, currentPage), cmdParms);
            }
        }
